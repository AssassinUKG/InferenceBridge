...
| IB-037 | Audit #18 safe mmproj auto-pairing | Done | Strongly named projectors still require a matching model-family token score. Standard generic files such as `mmproj-F32.gguf` are also accepted when they are colocated in an unambiguous dedicated model directory; recursive cross-repository pairing remains strict. Regression tests cover both the valid Qwen3.6 layout and an ambiguous mixed-model directory. Live validation on 2026-07-15 loaded `Qwen3.6-27B-UD-IQ3_XXS.gguf` with its colocated `mmproj-F32.gguf`; llama.cpp reported vision/video modalities and correctly read a controlled image. |
...
| IB-049 | Qwen3.6 unified multimodal detection | Done | Backend and frontend offline fallbacks now classify every Qwen3.6 variant as multimodal, including plain 27B GGUF filenames without `VL`, `vision`, or cached Hugging Face metadata. Runtime readiness remains a separate mmproj check. |
| IB-050 | Native multimodal API routing | Done | Image-bearing `/v1/chat/completions` requests now remain OpenAI content-part requests and proxy to llama.cpp's native multimodal endpoint. This avoids the obsolete `/completion` `[img-N]` plus `image_data` path that silently dropped visual input on current libmtmd builds. The route preserves scheduling, cancellation, status/content type, SSE, live traces, and usage metrics. Live JSON and SSE tests through public port 8800 both read `GREEN | HELIX 73` exactly with 252 prompt tokens. The original image-free control used the normalized route; IB-051 subsequently moved all effective `Qwen3_5` chat semantics to native Jinja. |
| IB-051 | Native Tess/Qwen3.5 chat routing | Done | Tess and all effective `Qwen3_5` chat-semantic surfaces now use llama.cpp's native `/v1/chat/completions` route. Desktop Chat preserves structured text/image history and reasoning; Chat Completions, Responses, and Messages additionally preserve structured tools and tool results. Embedded Jinja and load-time `--reasoning on/off` therefore apply consistently, while InferenceBridge records live Text, Think, tool-call, raw, usage, and completion events. Raw `/v1/completions` remains a raw-prompt compatibility surface. |
| IB-052 | Startup-safe context polling | Done | Context polling now verifies llama-server health before requesting `/slots`, so model startup/reload no longer emits a misleading 503 fallback warning while the backend is still loading. Healthy runtimes can still fall back to `/props` when `/slots` is genuinely unavailable. |
| IB-053 | Strict native 32K compaction | Done | Native Chat, Responses, Messages, and desktop Qwen3.5 routes compact complete conversation turns without orphaning assistant/tool exchanges, pin system/developer instructions and the newest turn, reserve the real requested output budget, account for images/tool schemas/response formats, and reject irreducible overflow clearly. |
| IB-054 | Public model-load control parity | Done | `/v1/models/load` now maps the load dialog's runtime controls, including GPU layers, threads/batches, Flash Attention, slots, Q8 KV types, unified KV, context shift, Jinja/reasoning/template choices, mmproj attachment, and explicit reload. Unknown flattened fields are no longer silently mistaken for active overrides. |
| IB-055 | Restart-safe model downloads | Done | Model Hub download metadata is written atomically to the app support directory. Startup reconciles saved jobs with the real `.part` length, automatically resumes transfers that were active at exit, preserves explicit pauses, and persists cancel, completion, discard, and Clear Done state. Download Manager merges restored state without overwriting newer live progress events. |
